...
export class BaseMpl {
  ...
  constructor(
    ...
    web3Config: { endpoint: string },
  ) {
    this.connection = new web3.Connection(web3Config.endpoint, {
  ...
  async createToken(input: CreateTokenParams) {
    ...
    const createTokenIxs = (
      await this.baseSpl.createToken({
        mintAuthority: user,
        decimal: decimals,
        freezAuthority: user,
  ...
  async getAllTokensWithMetadata(user: web3.PublicKey) {
    let allTokensRes = await this.baseSpl
      .getAllSplMints(user)
      .catch((getAllSplMintsError) => {
        log({ getAllSplMintsError });
        return null;
      });
    allTokensRes = allTokensRes ?? [];
    const metadataAddresses = allTokensRes.map((e) =>
      BaseMpl.getMetadataAccount(e.mint),
    );
    const accountInfoes = await getMultipleAccounts(
      this.connection,
      metadataAddresses,
    );
    const mainRes: {
